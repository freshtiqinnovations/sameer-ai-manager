...
def understand_text(text):
    ...
    if re.search(r'(check|status|health|dekh|dikha|dikhao|kitna|report|kya haal|kaisa|kaise|full|overview|summary|sab|all)', t):
        if re.search(r'(disk|space|storage|memory|hard)', t):
            ...
            return {"type": "ai_query", "query": text, "description": "AI query", "to_user": "AI answer ready..."}
    ...
    # 6. Check for casual chat / greeting
    t_lower = text.strip().lower()
    casual_words = {"hello", "hi", "hey", "bro", "boss", "haan", "ha", "yes", "ok", "okay", "thanks", "thx", "thankyou", "👍", "🙏", "🔥", "⚡", "😂", "😎"}
    if (t_lower in casual_words or 
        t_lower.rstrip('?').strip() in casual_words or
        t_lower.startswith("kaise ho") or
        t_lower.startswith("kya haal") or
        t_lower.startswith("sab thik") or
        t_lower in {"ji", "hanji", "hmm", "acha", "achha", "theek"}):
        answer = ask_ollama_first(text)
        return {
            "type": "chat_reply",
            "description": answer,
            "to_user": answer
        ...
        return {
            "type": "chat_reply",
            "description": "Haan bhai! Kya kaam hai? 😎",
            "to_user": "Haan bhai! Kya kaam hai? 😎"
        }

...
async def agent_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    return await update.message.reply_text("AI agent temporary active")
...
app.add_handler(CommandHandler("health", health))
app.run_polling(drop_pending_updates=True, read_timeout=60, write_timeout=60, connect_timeout=60, pool_timeout=60)
'''
...
async def ai_cmd(update, context):
    try:
        ...
        await update.message.reply_text("🤖 AI thinking...")
...
# === CUSTOMER CRM LOOKUP v1 ===
async def src_lookup_cmd(update, context):
    import re, json
    from pathlib import Path

    text = update.message.text or ""
    m = re.search(r"SRC\d+", text.upper())
    if not m:
        return

    src = m.group(0)
    base = Path(f"/root/customer_memory/customers/{src}")
    data = {}

    for f in [base / "ledger.json", base / "memory.json", base / "profile.json", base / "customer.json"]:
        if f.exists():
            try:
    ...
    files = []
    if base.exists():
        files = [x.name for x in base.rglob("*") if x.is_file()][:10]

    name = data.get("name") or data.get("customer_name") or data.get("account") or "Not saved"
    phone = data.get("phone") or data.get("mobile") or data.get("contact") or "Not saved"
    city = data.get("city") or data.get("location") or data.get("area") or "Not saved"
    balance = data.get("balance") or data.get("due") or data.get("outstanding") or "Not available"
    note = data.get("note") or data.get("notes") or data.get("summary") or "No notes saved"

    reply = (
        f"🏭 EL SALAMA CUSTOMER CRM\n"
        f"━━━━━━━━━━━━━━━\n"
        f"🆔 SRC: {src}\n"
        f"👤 Name: {name}\n"
        f"📞 Mobile: {phone}\n"
        ...
        f"🧠 Notes: {note}\n"
        f"━━━━━━━━━━━━━━━\n"
        f"📁 Files saved: {len(files)}\n"
    )

    if files:
        reply += "\n📎 Recent files:\n" + "\n".join([f"• {x}" for x in files[:5]])
